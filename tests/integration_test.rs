...
#[test]
fn iterate_archive_with_wrong_password() {
    let source = std::fs::File::open("tests/fixtures/with-password.zip").unwrap();
    let source_password = "wrong".into();
    ...
    assert_eq!(files_result.len(), 0);
    assert_eq!(files_result, empty_vec);
}
